...
# 超紧参数
# 翻牌前主动入池率：两人局&庄家位置，vpip水平16%左右，对应胜率0.596
# 位置调整vpip率：随着位置变差对应胜率依次变为，0.598，0.6，0.602，0.604，0.606，0.6085，位置每距离庄家位置+1，胜率阈值+0.002

# 紧凶参数
# 翻牌前主动入池率：两人局&庄家位置，vpip水平25%左右，对应胜率0.568
# 位置调整vpip率：随着位置变差（vpip率依次减少1%）对应胜率依次变为，0.565，0.562，0.559，0.557，0.554，0.551，位置每距离庄家位置+1，胜率阈值+0.002
strategy_data_v2_0 = [
    [
        0,  # pot
        [0.2, 0.568, 0.68, 1.01],  # winrate
        [Action.CHECK_CALL, Action.RAISE_HALF_POT, Action.RAISE_POT],
    ],
    [
        10,
        [0.35, 0.68, 0.7, 1.01],  # winrate  #弃牌快了一点吧？
        [Action.CHECK_CALL, Action.RAISE_HALF_POT, Action.RAISE_POT],
    ],
    [
        40,  # 池子大
...
strategy_data_v2_5 = [
    [
        0,  # pot
        [0.35, 0.65, 0.75, 1.01],  # winrate
        [Action.CHECK_CALL, Action.RAISE_HALF_POT, Action.RAISE_POT],
    ],
    [
        20,
        [0.45, 0.70, 0.9, 1.01],  # winrate  #可能过低
        [Action.CHECK_CALL, Action.RAISE_HALF_POT, Action.RAISE_POT],
    ],
    [
        40,
...
class GTOAgentV2(BaseAgent):
    def __init__(self, config=None):
        ...
        self.bluff_frequency = config.get("bluff_frequency", 0.0)
        ...
        self.valid_bluff_actions = config.get("valid_bluff_actions",
                                              [Action.RAISE_HALF_POT, Action.RAISE_POT, Action.ALL_IN])
        self.stage_map = {0: 0, 3: 1, 4: 2, 5: 3}
        self.raise_list = [Action.RAISE_HALF_POT, Action.RAISE_3_4_POT, Action.RAISE_POT, Action.RAISE_3_2_POT,
                           Action.RAISE_DOUBLE_POT]
        self.bet_threshold_table = [
            [0.0, 0.3, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 1.01],  # 胜率区间
            [10, 30, 100, 180, 300, 400, 500, 1000000]  # 对应最大下注金额
        ]
        # self.preflop_win_rate_adjust = [
        #     [3, 4, 5, 6,  7,  8,  9],  # 人数
        #     [0.17, 0.25, 0.3, 0.34, 0.36, 0.38, 0.40]  # 胜率调整系数
        # ]
    ...
    def step(self, state: State):
        ...
        n_position = (n_players - state.my_id + state.dealer_id) % n_players
        chips_remain = my_role.chips_remain
        chips_to_desk = my_role.chips_to_desk

        final_action = self.decide_action(hands, public_cards, n_position, pot, chips_to_desk, chips_remain, n_players,
                                          legal_actions, history_seq, state.my_id, state.small_blind, state)
    ...
    def apply_position_bias(self, sd, n_position, n_players):
        result = []
        print(f'n_position:{n_position}')
        for item in sd:
            if len(item) == 1:  # 如 [100000]
                result.append(item)
            elif len(item) == 3:  # [pot, winrates, actions]
                pot, winrates, actions = item
                if len(winrates) >= 2:
                    new_winrates = winrates.copy()  # 浅拷贝
                    original_val = winrates[1]
                    # vpip增加0.5%，对应0.002；增加1%对应0.003
                    new_val = original_val + 0.003 * n_position - 0.003 * (n_players - 2)
                    new_val = max(0.0, min(1.0, new_val))
                    new_winrates[1] = new_val
                    result.append([pot, new_winrates, actions])
                else:
                    result.append(item)
            else:
                result.append(item)  # 其他未知结构，保留原样
        return result
    ...
    def parse_history(self, history_seq, com_cards_size, my_id):
        ...
        for act in history_seq:
            if act.player_id == my_id and act.action >= 0 and Action(act.action) in self.raise_list:
                total_raise_cnt += 1
            if act.player_id == my_id and act.action >= 0 and act.stage == self.stage_map[com_cards_size] and Action(
                    act.action) in self.raise_list:
    ...
    def decide_action(self, hands, public_cards, n_position, pot, chips_to_desk, chips_remain, n_players=2,
                      legal_actions=None, history_seq=None, my_id=None, small_blind=None, state=None) -> Action:
        ...
        # preflop时，noisy_win_rate根据人数做调整
        # if len(public_cards)<3:
        # print(f'before adjust rate:{noisy_win_rate}')
        # print(n_players)
        # idx = (n_players-3)
        # adjustment = self.preflop_win_rate_adjust[1][idx]
        # noisy_win_rate -= adjustment
        # print(f'after adjust rate:{noisy_win_rate}')
        ...
        # alive player
        n_alive_player = n_players - fold_cnt
        pot = pot / n_alive_player
        # 当套池率很高时，增加胜率以提升raise比例
        if len(public_cards) >= 3:
            print(f'chips_remian:{chips_remain}')
            print(f'chips_to_desk:{chips_to_desk}')
            if chips_to_desk / (chips_remain + chips_to_desk) >= 0.8:
                print(f'before_high_chip_rate_adjust_winrate:{noisy_win_rate}')
                noisy_win_rate = min(1.0, noisy_win_rate + 0.25)
                print(f'high_chip_rate_adjust_winrate:{noisy_win_rate}')
            if chips_remain <= 2 * small_blind:
                noisy_win_rate = min(1.0, noisy_win_rate + 0.5)
                #print(f'below2bb_after_adjust_win_rate:{action}')
        # === 正常决策流程 ===
        if len(public_cards) < 3:  # preflop
            action = Action.FOLD
            # print(f'original sd:{sd}')
            biased_sd = self.apply_position_bias(sd, n_position, n_players)
            # print(f'adjusted_sd:{biased_sd}')
            for i in range(len(sd) - 1):
                if biased_sd[i][0] < pot <= biased_sd[i + 1][0]:
                    # print(f'check pot{pot},{sd[i][0]},{sd[i+1][0]}')
                    for j in range(len(biased_sd[i][1]) - 1):
                        if biased_sd[i][1][j] <= noisy_win_rate < sd[i][1][j + 1]:
                            action = biased_sd[i][2][j]
                            # print(f'action:{action}')

            # 翻牌前特殊逻辑：统计全场所有玩家（不区分对手和自己）的 raise 次数
            # 如果本轮已经有 >=2 次 raise，则大幅降低再次 raise 的概率
            current_mapped_stage = self.stage_map.get(current_phase, 0)
            stage_total_raise_cnt = 0
            for act in history_seq:
                if act.stage == current_mapped_stage and act.action >= 0:
                    # 只要是 raise 或 all-in 都算，不判断 player_id
                    if Action(act.action) in self.raise_list or Action(act.action) == Action.ALL_IN:
                        stage_total_raise_cnt += 1
                        print(f'stage_total_raise_cnt:{stage_total_raise_cnt},action:{action}')
            if stage_total_raise_cnt >= 2 and (action in self.raise_list or action == Action.ALL_IN):
                # 只有 10% 的概率保持 raise，90% 概率转为 check/call
                if random.random() < 0.95:
                    action = Action.CHECK_CALL
                    print(f'stage_total_raise_cnt:{stage_total_raise_cnt},action2:{action}')

        else:  # postflop GTO
            # noisy_win_rate=(noisy_win_rate+0.05)*noisy_win_rate^(n_alive_player-2)
        ...
        # == raise 多样性
        if action in [Action.RAISE_HALF_POT, Action.RAISE_3_4_POT, Action.RAISE_POT, Action.RAISE_3_2_POT,
                      Action.RAISE_DOUBLE_POT]:
