...
class NolimitholdemJudger:
    ...
    def judge_game(self, players, hands, game_type):
        """
        ...
        payoffs = [0] * len(hands)
        # 只有1个人没有fold的场景
        if fold_count == len(hands) - 1:
            for i in range(len(hands)):
                if hands[i] is not None:
                    payoffs[i] = remaining - in_chips[i]
                else:
                    payoffs[i] = -in_chips[i]

            assert sum(payoffs) == 0
            return payoffs

        while remaining > 0:
            winners = compare_hands(hands, game_type)
