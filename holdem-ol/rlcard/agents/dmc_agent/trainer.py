...
class DMCTrainer:
    ...
    def start(self):
        ...
        def batch_and_learn(idx, dev, pos, local_lock, position_lock, lock=threading.Lock()):
            """Thread target for the learning process."""
            nonlocal frames, stats
            # idx cloud be used later
            _ = idx
        ...
        def checkpoint(_frames):
            ...
            for pos in range(self.num_players):
                model_weights_dir = os.path.expandvars(os.path.expanduser(
                    '%s/%s/%s' % (self.savedir, self.xpid, str(pos) + '_' + str(_frames) + '.pth')))
                torch.save(
                    learner_model.get_agent(pos),
                    model_weights_dir
                )
