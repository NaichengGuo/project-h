...
def create_buffers_pettingzoo(
    ...
    for device in device_iterator:
        ...
        for agent_name in env.agents:
            ...
            specs = dict(
                done=dict(size=(tensor_size,), dtype=torch.bool),
                episode_return=dict(size=(tensor_size,), dtype=torch.float32),
                target=dict(size=(tensor_size,), dtype=torch.float32),
                state=dict(size=(tensor_size,) + tuple(state_shape), dtype=torch.int8),
                action=dict(size=(tensor_size,) + (env.action_space(agent_name).n,), dtype=torch.int8),
            )
    ...
    try:
        ...
        while True:
            ...
            for agent_id, agent_name in enumerate(env.possible_agents):
                ...
                while size[agent_id] > tensor_size:
                    index = free_queue[agent_id].get()
                    if index is None:
                        print("index is None")
                        break
                    for t in range(tensor_size):
                    ...
                    done_buf[agent_id] = done_buf[agent_id][tensor_size:]
                    episode_return_buf[agent_id] = episode_return_buf[agent_id][tensor_size:]
                    target_buf[agent_id] = target_buf[agent_id][tensor_size:]
                    state_buf[agent_id] = state_buf[agent_id][tensor_size:]
                    action_buf[agent_id] = action_buf[agent_id][tensor_size:]
                    size[agent_id] -= tensor_size
