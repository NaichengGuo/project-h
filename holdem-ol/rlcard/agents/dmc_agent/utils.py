    ...
    for device in device_iterator:
        ...
        for player_id in range(len(state_shape)):
            specs = dict(
                ...
                state=dict(size=(tensor_size,) + tuple(state_shape[player_id]), dtype=torch.int8),
                action=dict(size=(tensor_size,) + tuple(action_shape[player_id]), dtype=torch.int8),
            )
        ...
        while True:
            ...
            for p in range(env.num_players):
                ...
                while size[p] > tensor_size:
                    ...
                    done_buf[p] = done_buf[p][tensor_size:]
                    episode_return_buf[p] = episode_return_buf[p][tensor_size:]
                    target_buf[p] = target_buf[p][tensor_size:]
                    state_buf[p] = state_buf[p][tensor_size:]
                    action_buf[p] = action_buf[p][tensor_size:]
                    size[p] -= tensor_size
