...
def _get_mlp_layers_with_default(layers):
    if layers is None:
...
class DMCNet(nn.Module):
    def __init__(
        ...
        layer_dims = [input_dim] + _get_mlp_layers_with_default(mlp_layers)
        fc = []
...
class DMCAgent:
    def __init__(
        ...
        self.net = DMCNet(state_shape, action_shape, _get_mlp_layers_with_default(mlp_layers)).to(self.device)
    ...
    def __init__(
        ...
        for player_id in range(len(state_shape)):
            agent = DMCAgent(
                ...
                _get_mlp_layers_with_default(mlp_layers),
                exp_epsilon,
                device,
            )
