...
def calculate_win_rate(player1_hand, community_cards, num_trials=1000, game_type: GameType = GameType.HOLDEM):
    remain_desk = [card for card in CardCode.all_card_strs if card not in (player1_hand + community_cards)]
    wins = 0
    for _ in range(num_trials):

        random_cards = random.sample(remain_desk, 7)
        player2_hand = random_cards[:2]

        if len(community_cards) < 5:
            all_community_cards = community_cards + random_cards[2:2 + (5 - len(community_cards))]
        else:
        ...
        winner = compare_hands([player_1_all_cards, player_2_all_cards], game_type)
...
class Hand:
    def __init__(self, all_cards, game_type):
        ...
        self.SUIT_LOOKUP = "SCDH" # Suit of a card  Spade, Club, Diamond, Heart  黑桃，梅花，方块，红心

        self.game_type = game_type
        if self.game_type == GameType.HOLDEM:
            self.full_house_category = 7
            self.flush_category = 6
            self.A_alternate_value = 1
        else:
            self.full_house_category = 6
            self.flush_category = 7
            self.A_alternate_value = 5
    ...
    def evaluateHand(self):
        ...
        if self._has_straight_flush():
            ...
            self.category = self.full_house_category
            # Full house
            self.best_five = self._get_Fullhouse_cards()
        elif self._has_flush():
            self.category = self.flush_category
            # Flush
    ...
    def is_full_house_category(self):
        return self.category == self.full_house_category

    def is_flush_category(self):
        return self.category == self.flush_category
    ...
    def _get_straight_cards(self, Cards):
        ...
        if highest_card[1] == 'A':
            ...
            ranks.insert(0, self.A_alternate_value)
...
def compare_hands(hands, game_type: GameType):
    """
        ...
        if len(fold_players) == len(all_players) - 1:
            ...
            for _ in enumerate(hands):
                if hands[_[0]] is not None:
                    hand = Hand(hands[_[0]], game_type)
                    hand.evaluateHand()
                    hand_category.append(hand.category)
                    hand_eval.append(hand)
                elif hands[_[0]] is None:
                    hand_category.append(0)
                    hand_eval.append(None)
    else:
        for i in enumerate(hands):
            hand = Hand(hands[i[0]], game_type)
            hand.evaluateHand()
            hand_category.append(hand.category)
            hand_eval.append(hand)
    # potential_winner_index = [i for i, j in enumerate(hand_category) if
    #                           j == max(hand_category)]  # potential winner are those with same max card_catagory

    max_category = max(hand_category)
    potential_winner_index = []
    equal_hands = []
    for i in range(len(hand_category)):
        if hand_category[i] == max_category:
            # potential winner are those with same max card_category
            potential_winner_index.append(i)
            equal_hands.append(hand_eval[i])

    return final_compare(hands, potential_winner_index, all_players, equal_hands)


def final_compare(hands, potential_winner_index, all_players, equal_hands):
    """
    ...
    if len(potential_winner_index) == 1:
        ...
        if hand.is_full_house_category():
        ...
        if hand.category == 1 or hand.is_flush_category():
