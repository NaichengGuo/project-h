...
def ParseActionV2(sb_pos, action):
    stage = 0
    sz = len(action)
    bb_pos = (sb_pos + 1) % 2
    cur_pos = sb_pos
    actions = [(sb_pos, -1, "sb", 50), (bb_pos, -1, "bb", 100)]

    stage_chip_in_pot = {sb_pos: SMALL_BLIND, bb_pos: BIG_BLIND}
    stage_chip_infos = {}
    stage_chip_infos[stage] = stage_chip_in_pot

    if sz == 0:
        return actions, stage_chip_infos
    i = 0

    while i < sz:
        c = action[i]
        opponent_pos = (cur_pos + 1) % 2
        i += 1
        if c == '/':
            stage = stage + 1
            cur_pos = bb_pos
            stage_chip_in_pot = {sb_pos: 0, bb_pos: 0}
            stage_chip_infos[stage] = stage_chip_in_pot
        elif c == 'k':
            actions.append((cur_pos, stage, "k", 0))
            cur_pos = opponent_pos
        elif c == 'c':
            actions.append((cur_pos, stage, "c", stage_chip_in_pot[opponent_pos] - stage_chip_in_pot[cur_pos]))
            stage_chip_in_pot[cur_pos] = stage_chip_in_pot[opponent_pos]
            cur_pos = opponent_pos
        elif c == 'b':
            j = i
            ...
            diff = new_street_last_bet_to - stage_chip_in_pot[cur_pos]
            stage_chip_in_pot[cur_pos] = new_street_last_bet_to
            actions.append((cur_pos, stage, "b", diff))
            cur_pos = opponent_pos
        elif c == 'f':
            actions.append((cur_pos, stage, "f", 0))
            cur_pos = opponent_pos
    return actions, stage_chip_infos
...
def ParseAction(action):
    ...
    if sz == 0:
        return {
            'st': st,  # 阶段，0表示preflop，1表示flop，2表示turn，3表示river
            'pos': pos,
            'street_last_bet_to': street_last_bet_to,
            # bxxx 后面的数值，如 b600 这个值就是600，代表这一轮（这里的轮，指的是preflop/flop/turn/river这种局内的轮）操作者的总下注额
            'total_last_bet_to': total_last_bet_to,  # 本局自己的总下注额（包含了所有轮的）
            'last_bet_size': last_bet_size,  # 2次street_last_bet_to的差值
            'last_bettor': last_bettor,  # 上次下注的者的位置
        }
...
def main():
    ...
    parser.add_argument('--username', type=str,default='julian123')
